...
10. `v` でキュービューアーを開き、追加したトラック一覧が表示されることを確認。`↑`/`↓` で選択し `d` で個別削除、`Shift+↑`/`Shift+↓` で並び替えできることを確認。`Esc` で閉じることを確認
11. `c` でキューをクリアし、タイトルバーが元に戻ることを確認
12. `S` を押すたびにタイトルバーのソートキーが `Title → Artist → Album → Duration → Default` と切り替わり、トラックリストが並び替わることを確認。再生中のトラックがずれないことを確認
13. `r` を押すたびに下部バーのリピートモードが `Off → All → One → Off` と切り替わることを確認
14. `s` でプレイリストが `~/.config/crabplay/playlists/` に保存され、Now Playing に保存先が表示されることを確認
15. `o` で SourcePicker を開き `[Dir]` → `[Recent]` → `[PL]` の順に表示されることを確認。別ディレクトリをロード後に再度 `o` を開くと `[Recent]` に追加されることを確認。`[Recent]` エントリを選択して `d` を押すと履歴から削除され `config.toml` に反映されることを確認。`[PL]` エントリを選択して `d` を押すとファイルが削除されることを確認
16. `/` キーで検索モードに入り、キーワードを入力するとトラックリストがリアルタイムにフィルタリングされることを確認。`Enter` で選択確定、`Esc` でキャンセルして元の選択に戻ることを確認
17. `?` キーでヘルプオーバーレイが開くことを確認。↑/↓ でスクロールできることを確認。任意のキーで閉じてトラックリストに戻ることを確認
18. `T` キーでスリープタイマー入力オーバーレイが開くことを確認。分数（例: `1`）を入力して `Enter` を押し、タイトルバーに `[Timer: 01:00]` が表示されることを確認。時間経過後に再生が停止し通知が表示されることを確認。`T` → 空 `Enter` でキャンセルできることを確認
19. `+/-` で音量を変更・`r` でリピートモードを変更・`z` でシャッフルをオンにして `q` で終了。再起動後に同じ設定が復元されることを確認（`~/.config/crabplay/config.toml` の内容も確認）
20. `q` で終了し、ターミナルが正常に復元されることを確認
