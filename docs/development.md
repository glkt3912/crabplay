...
8. `a` でトラックをキューに追加し、タイトルバーに `[queue: N]` が表示されることを確認
9. `c` でキューをクリアし、タイトルバーが元に戻ることを確認
10. `r` を押すたびに下部バーのリピートモードが `Off → All → One → Off` と切り替わることを確認
11. `s` でプレイリストが `~/.config/crabplay/playlists/` に保存され、Now Playing に保存先が表示されることを確認
12. `q` で終了し、ターミナルが正常に復元されることを確認
