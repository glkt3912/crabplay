...
12. `r` を押すたびに下部バーのリピートモードが `Off → All → One → Off` と切り替わることを確認
13. `s` でプレイリストが `~/.config/crabplay/playlists/` に保存され、Now Playing に保存先が表示されることを確認
14. `o` で SourcePicker を開き `[Dir]` → `[Recent]` → `[PL]` の順に表示されることを確認。別ディレクトリをロード後に再度 `o` を開くと `[Recent]` に追加されることを確認。`[Recent]` エントリを選択して `d` を押すと履歴から削除され `config.toml` に反映されることを確認。`[PL]` エントリを選択して `d` を押すとファイルが削除されることを確認
15. `/` キーで検索モードに入り、キーワードを入力するとトラックリストがリアルタイムにフィルタリングされることを確認。`Enter` で選択確定、`Esc` でキャンセルして元の選択に戻ることを確認
16. `?` キーでヘルプオーバーレイが開くことを確認。↑/↓ でスクロールできることを確認。任意のキーで閉じてトラックリストに戻ることを確認
17. `+/-` で音量を変更・`r` でリピートモードを変更・`z` でシャッフルをオンにして `q` で終了。再起動後に同じ設定が復元されることを確認（`~/.config/crabplay/config.toml` の内容も確認）
18. `q` で終了し、ターミナルが正常に復元されることを確認
